...
//!
//! Inclusion-check counterpart to `staccana_genesis::merkle::MerkleTree::build`. Given a
//! `(holder_pubkey, total_allocation_lamports)` leaf and a sibling-hash path, recompute
//! the root and compare against the embedded value in `MegadropConfig.claimable_root`.
//!
//! ## Domain bytes — MUST stay in sync with `staccana_genesis::merkle`
//!
//! Both crates use SHA-256 (via `solana_program::hash::hashv`) with explicit domain
//! bytes to prevent leaf/internal-node second-preimage attacks:
//!
...
//! megadrop crate has no dependency on the genesis-builder crate. If the genesis side
//! ever changes either constant, this file MUST change to match — the snapshot tool
//! `tools/megadrop-snapshot` produces the root using `staccana_genesis::merkle`, so
//! the cross-crate root equality is effectively the integration test.
//!
//! ## Proof flag encoding
//!
//! `proof_flags` is a packed bitfield over `proof.len()` bits, LSB-first within each
//! byte:
//!
//! * bit i = 0 → sibling is on the LEFT of the running hash; combine as
//!   `node(sibling, running)`
//! * bit i = 1 → sibling is on the RIGHT of the running hash; combine as
//!   `node(running, sibling)`
//!
//! Trailing bits in the final byte beyond `proof.len()` are ignored.

use solana_program::hash::{hashv, Hash};

/// Domain-separation byte for leaf hashes — must match `staccana_genesis::merkle::LEAF_DOMAIN`.
pub const LEAF_DOMAIN: u8 = 0x00;
/// Domain-separation byte for internal node hashes — must match
/// `staccana_genesis::merkle::NODE_DOMAIN`.
pub const NODE_DOMAIN: u8 = 0x01;

/// Compute the leaf hash for a `(holder_pubkey, total_allocation_lamports)` claim.
/// Identical hashing to `staccana_genesis::merkle::ClaimableLeaf::hash` (different
/// semantics — pubkey here is the holder, lamports is the total allocation, not a
/// system-account balance).
pub fn leaf_hash(holder_pubkey: &[u8; 32], total_allocation: u64) -> Hash {
    hashv(&[
        &[LEAF_DOMAIN],
        holder_pubkey.as_ref(),
        &total_allocation.to_le_bytes(),
    ])
}
...
mod tests {
    ...
    /// Build a tiny tree manually. Mirrors the algorithm in
    /// `staccana_genesis::merkle::MerkleTree::build`.
    ...
    /// Construct a proof + flag set for `target_idx`. Returns `(proof, packed_flags)`.
    ...
    fn verify_succeeds_for_valid_proof() {
        ...
        for (i, (p, l)) in leaves.iter().enumerate() {
            let idx = sorted_index_of(&leaves, p);
            let (proof, flags) = build_proof(&leaves, idx);
            let leaf = leaf_hash(p, *l);
            assert!(
                verify_inclusion(leaf, &proof, &flags, &root),
                "leaf {} (alloc {}) failed verification",
                i,
                l
            );
        }
    ...
    #[test]
    fn verify_fails_for_wrong_total_allocation() {
        let leaves = vec![(pk(1), 100u64), (pk(2), 200u64)];
        let root = build_root(&leaves);
        let (proof, flags) = build_proof(&leaves, 0);

        // Right pubkey, wrong total — claim someone is owed more than they are.
        let bad_leaf = leaf_hash(&pk(1), 999);
        assert!(!verify_inclusion(bad_leaf, &proof, &flags, &root));
    }

    #[test]
    fn verify_fails_for_wrong_holder_pubkey() {
        let leaves = vec![(pk(1), 100u64), (pk(2), 200u64)];
        let root = build_root(&leaves);
        let (proof, flags) = build_proof(&leaves, 0);

        // Right total, wrong holder — try to redirect another holder's claim.
    ...
    #[test]
    fn leaf_hash_uses_le_encoding_for_total_allocation() {
        // Ensure the lamports field is encoded as 8 bytes LE so the on-chain leaf hash
        // matches what `staccana_genesis::merkle::ClaimableLeaf::hash` produces. Pin
        // a known-byte hash to catch any encoding regression.
        let pk_bytes = [0xABu8; 32];
        let total: u64 = 0x0102_0304_0506_0708;
        let computed = leaf_hash(&pk_bytes, total);

        // Recompute the expected hash byte-for-byte using `hashv` directly with the
        // documented preimage layout.
        let expected = hashv(&[
            &[LEAF_DOMAIN],
            &pk_bytes,
            &total.to_le_bytes(),
        ...
        assert_eq!(computed.as_ref(), expected.as_ref());
    }
}
