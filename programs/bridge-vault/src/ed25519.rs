...
/// Hard cap on signatures in one batched precompile ix. Mirrors the federation
/// cap on the staccana side.
pub const MAX_BATCH_SIGS: usize = crate::state::MAX_FEDERATION_MEMBERS;

/// Decode a batched ed25519 precompile ix carrying multiple signatures over a
/// shared (or per-sig) message. See `programs/bridge/src/ed25519.rs` for the
/// rationale — this is the lockstep mirror so the burn → release path can use the
/// same compact tx encoding the deposit → mint path now uses.
///
/// Each `ParsedEd25519` returned is self-contained — `(pubkey, signature, message)`
/// all reference into THIS ix's data, never cross-ix. Cross-ix references are
/// rejected to keep the verifier semantics simple.
pub fn parse_ed25519_batch_at(
    sysvar: &AccountInfo,
    index: usize,
) -> std::result::Result<Vec<ParsedEd25519>, VaultError> {
    let ix = load_instruction_at_checked(index, sysvar)
        .map_err(|_| VaultError::BadEd25519Precompile)?;
    if ix.program_id != ed25519_program::ID {
        return Err(VaultError::BadEd25519Precompile);
